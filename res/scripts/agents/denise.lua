...
local msg_handlers = {
    ["mark_start"] = function(self, msg_data)
        ...
        if not sound.is_busy(Channel.SOUL01) then
            sound.play("denise/reactions/mark_greet/*", Channel.SOUL01, { interrupt = false })
        end
    end,
    ["mark_talk"] = function(self, msg_data)
        ...
        if not sound.is_busy(Channel.SOUL01) then
            sound.play("denise/reactions/mark_plead/*", Channel.SOUL01, { interrupt = false })
        end
    end,
    ["mark_end"] = function(self, msg_data)
        ...
        if not sound.is_busy(Channel.SOUL01) then
            sound.play("denise/reactions/mark_post/*", Channel.SOUL01, { interrupt = false })
        end
    end
}
...
module:state(AgentState.CALL, {
    enter = function(self)
        ...
        for i = 1, rand_int(1, 3) do
            sound.play_wait("denise/seeing/*", Channel.PHONE01, { interrupt = true })
